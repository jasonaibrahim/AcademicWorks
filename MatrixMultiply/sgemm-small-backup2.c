...
void mm( int n, float *An, float *Cn)
{
	...
	{
		a0 = _mm_loadu_ps(An+blockInd1+k*n);
		a1 = _mm_loadu_ps(An+blockInd1+4+k*n);
		a2 = _mm_loadu_ps(An+blockInd1+8+k*n);
		a3 = _mm_loadu_ps(An+blockInd1+12+k*n);
		a4 = _mm_loadu_ps(An+blockInd1+16+k*n);

		for(j = 0; j < n; j++)
		{
			a0T = _mm_load1_ps(An+j+k*n);

			for(i = blockInd1; i < blockInd1 + BLOCKSIZE; i+=BLOCKSIZE)
			{

			//	a1 = _mm_loadu_ps(An+(i+4+k*n));
			//	temp1 = _mm_mul_ps(a0, a0T);

			//	a2 = _mm_loadu_ps(An+(i+8+k*n));
			//	temp2 = _mm_mul_ps(a1, a0T);

			//	a3 = _mm_loadu_ps(An+(i+12+k*n));
			//	temp3 = _mm_mul_ps(a2,a0T);

			//	a4 = _mm_loadu_ps(An+(i+16+k*n));
			//	temp4 = _mm_mul_ps(a3, a0T);
			...
			//	a4 = _mm_mul_ps(a4, a0T);
				...
				c0 = _mm_add_ps(c0, temp1);
				temp3 = _mm_mul_ps(a2, a0T);

				c2 = _mm_loadu_ps(Cn+i+8+j*n);
				c1 = _mm_add_ps(c1, temp2);
				temp4 = _mm_mul_ps(a3, a0T);

				c3 = _mm_loadu_ps(Cn+i+12+j*n);
				c2 = _mm_add_ps(c2,temp3);
				
				_mm_store_ps(Cn+i+4+j*n, c1);
				temp1 = _mm_mul_ps(a4, a0T);
				
				c4 = _mm_loadu_ps(Cn+i+16+j*n);
				c3 = _mm_add_ps(c3,temp4);

				_mm_storeu_ps(Cn+i+j*n,c0);
				c4 = _mm_add_ps(c4, temp1);
